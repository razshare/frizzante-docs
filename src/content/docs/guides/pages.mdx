...
you must manually load into on the
server using `.LoadController()` during package initialization.
...
This will load the current package, in this case `welcome`, as a controller.

### Any

The `any` package has a special meaning.

It is treated as a fallback controller, meaning it will handle any
request that doesn't match any other controller.
