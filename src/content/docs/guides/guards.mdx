...
func main() {
    ...
    srv.Guards = []srv.Guards{
        {                                                             // Adds guard.
            ...
            Handler: func(client *client.Client, allow func()) {      // Sets guard handler.
               if receive.ContentType(client) == "application/json" { // Checks content type.
...
src.Guards = []guard.Guards{
   {                                                              // Adds authentication guard.
        ...
        Handler: func(client *client.Client, allow func()) {      // Sets guard handler. Call allow() in order to permit the request to
                                                                  // advance to the next guard or to the route handler.
            state := session.Start(receive.SessionId(client))     // Starts session.
            if state.Verified &&                                  // Checks if client is verified. This "Verified" field is
                                                                  // usually set by a public route, like a /login page.
               time.Since(state.LastActivity) <= 30*time.Minute { // Checks if client has been active in the past 30 minutes.
               allow()                                            // Allows the request through.
               return
            }
            send.Status(client, 401)                              // Hints to client that authentication is missing.
            send.Message(client, "not authenticated")             // Sends a readable hint message.
    ...
    {                                                             // Adds authorization guard.
        ...
        Handler: func(client *client.Client, allow func()) {      // Sets guard handler.
            state := session.Start(receive.SessionId(client))     // Starts session.
            if state.UserId == receive.path("user_id") {          // Checks if session user id matches requested user id.
               allow()                                            // Allows the request through.
               return
            }
            send.Status(client, 401)                              // Hints to client that authorization is missing.
            send.Message(client, "not authorized")                // Sends a readable hint message.
            return                                                // At this point allow() has not been called, so the request is blocked.
        },
