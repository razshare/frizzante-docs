...
Guards tags and routes tags can intersect.

When a route intersects with a guard tag, that guard is applied to the route.

In this case, the `GET /` route intersects with the `NoJson` guard, which means route `GET /` is protected by `NoJson`.
...
src.Guards = []guard.Guards{
   {                                                          // Adds authentication guard.
        ...
        Tags: []tag.Tag{authentication, auth},                // Tags the guard - use any of these tags to apply this guard to any route.
    },
    {                                                         // Adds authorization guard.
        ...
        Tags: []tag.Tag{authorization, auth},                 // Tags the guard - use any of these tags to apply this guard to any route.
    },
}
