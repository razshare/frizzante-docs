...
func main() {
    server.AddGuard(guards.Guard{                                  // Adds a guard.
        Name: "No Json Allowed",                                   // Names the guard, this will appear in your notifier log.
        Handler: func(con *connections.Connection, allow func()) { // Adds logic to the guard.
            if con.VerifyContentType("application/json") {         // Rejects "application/json" requests...
                return
            }
            allow()                                                // ...allows other requests.
        },
        Tags: []string{"nojson", "jsonless"},                      // Sets the guard's tags.
    })
    server.AddRoute(routes.Route{                                  // Adds a route.
        Pattern: "GET /",                                          // Maps the route to a pattern.
        Handler: func(con *connections.Connection) {
            con.SendMessage("Hello.")                              // Sends text.
        },
        Tags: []string{"nojson"},                                  // Sets the routes's tags.
    })
