...
:::tip
If the view **Name** is not present in neither `app/exports.server.ts` or `app/exports.client.ts`, `send.View()` will resolve into a **Noop**.

The default starter template comes with a client and server routing configuration 
out of the box, which you can find in [app/frizzante/core/components/*](https://main/lib/core/tree/main/template/project/app/frizzante/core/components).
...
import (
    "embed"
    "main/lib/core/client"
    "main/lib/core/svelte/container/server"
)
