...
Before rendering views you need to replace your **server** with a **container server**.
...
var srv = server.New(efs)   // Creates container server.

func main() {
    defer server.Start(srv) // Starts server.
    srv.Server.Routes = []route.Route{}
}
```
