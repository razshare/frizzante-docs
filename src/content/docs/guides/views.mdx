---
title: Views
---

import { FileTree, Tabs, TabItem } from '@astrojs/starlight/components'

Views are **svelte components** exported by 
`app/lib/exports/server.ts` and/or `app/lib/exports/client.ts`.

<FileTree>
    - main.go
    - app
        - lib
            - exports
                - client.ts
                - server.ts
                - ...
</FileTree>

<Tabs>
    <TabItem icon='seti:typescript' label='Server exports'>
    Views that are meant to be rendered on the server should be exported by this file.
    ```ts
    //app/lib/exports/server.ts
    import Welcome from '$lib/components/views/Welcome.svelte'
    import Profile from '$lib/components/views/Profile.svelte'
    import type {Component} from "svelte";

    export const views: Record<string, Component> = {
        "Welcome": Welcome,
        "Profile": Profile,
    }
    ```
    </TabItem>
    <TabItem icon='seti:typescript' label='Client exports'>
    Views that are meant to be rendered on the client should be exported by this file.
    ```ts
    //app/lib/exports/client.ts
    export const views: Record<string, Promise<unknown>> = {
        "Welcome": import('$lib/components/views/Welcome.svelte'),
        "Profile": import('$lib/components/views/Profile.svelte'),
    }
    ```
    
    These views are being imported asynchronously in order to split them 
    in different bundles, however you can simply create fake promises
    in order to bundle them all together and eliminate network latency
    when transitioning between views.
    
    ```ts
    //app/lib/exports/client.ts
    import Welcome from '$lib/components/views/Welcome.svelte'
    import Profile from '$lib/components/views/Profile.svelte'
    import type {Component} from "svelte";

    export const views: Record<string, Promise<unknown>> = {
        "Welcome": Promise.resolve(Welcome),
        "Profile": Promise.resolve(Profile),
    }
    ```
    </TabItem>
</Tabs>

## Configuring server

In order for the server to be able to serve views you must configure it first
by specifying a **public root** directory, a **view server** and a **view index**.

##### Public root

Use `WithPublicRoot()` to configure the server's public root directory,
which is usually `app/dist/client`.

```go
package main

import "github.com/razshare/frizzante/frz"


func main() {
    frz.NewServer().
        WithPublicRoot("app/dist/client").
        Start()
}
```

##### View server

Use `WithViewServer()` to configure the view server,
which is usually `app/dist/server.js`.

```go
package main

import "github.com/razshare/frizzante/frz"


func main() {
    frz.NewServer().
        WithPublicRoot("app/dist/client").
        WithViewServer("app/dist/server.js").
        Start()
}
```

##### View index

Use `WithViewIndex()` to configure the view index file,
which is usually `app/dist/client/index.html`.

```go
package main

import "github.com/razshare/frizzante/frz"


func main() {
    frz.NewServer().
        WithPublicRoot("app/dist/client").
        WithViewServer("app/dist/server.js").
        WithViewIndex("app/dist/client/index.html").
        Start()
}
```

## Send views

Use `SendView()` to send a view from a route handler.

```go
//lib/routes/handler.go
package lib

import "github.com/razshare/frizzante/frz"

func Hello(c *frz.Connection) {
    // Sends file "lib/components/views/Hello.svelte"
    c.SendView(frz.View{Name: "Hello"}) 
}
```

The **Name** of the view will be used to lookup the view component exported by **server.ts** and/or **client.ts**.

## Default view

There is no way to specify a "**default view**".

However, you can use `SendFileOrElse()` in order to send a file or run custom logic if it doesn't exist.

```go
//lib/routes/handler.go
package lib

import "github.com/razshare/frizzante/frz"

func Hello(c *frz.Connection) {
    c.SendFileOrElse(func () {
        c.SendView(frz.View{Name: "Hello"})
    })
}
```

Usually you would map this handler to the default `GET /` pattern, which automatically captures
all un matched requests.

```go
package main

import (
    "embed"
    "github.com/razshare/frizzante/frz"
)

//go:embed app/dist
var efs embed.FS

func main() {
    frz.NewServer().
        WithEfs(efs).
        WithPublicRoot("app/dist/client").
        WithViewServer("app/dist/server.js").
        WithViewIndex("app/dist/client/index.html").
        AddRoute(frz.Route{Pattern: "GET /", Handler: handlers.Hello}).
        Start()
}
```

## Data

Optionally, you can send data along with the views by adding a `Data` property to your `View`.

```go
//lib/routes/handler.go
package lib

import "github.com/razshare/frizzante/frz"

func Hello(c *frz.Connection) {
    c.SendView(frz.View{ 
        Name: "Hello",
        Data: map[string]string{
            "name": "world",
        }
    }) 
}
```

Then use [getContext("server")](https://svelte.dev/docs/svelte/context) to retrieve your data.

```svelte
//lib/components/views/Hello.svelte
<script lang="ts">
    import type { ServerContext } from "$frizzante/types.ts"
    const server = getContext("server") as ServerContext<{ name: string }>
</script>

<h1>Hello {server.data.name}</h1>
```

## Render modes

You can choose how to render views with the `RenderMode` property.

<Tabs>
    <TabItem label='RenderModeFull'>
        The view is rendered on both the server and the client.<br/>
        This is the **default** mode.
        ```go
        //lib/routes/handler.go
        package lib

        import "github.com/razshare/frizzante/frz"

        func Hello(c *frz.Connection) {
            c.SendView(frz.View{
                Name: "Hello",
                RenderMode: frz.RenderModeFull,   // Renders on server and client.
            })
        }
        ```
    </TabItem>
    <TabItem label='RenderModeServer'>
        The view is rendered only on the server.<br/>
        You'll have to deal away with apis such as [fetch](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API); 
        your new best friend is [form](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/form).
        ```go
        //lib/routes/handler.go
        package lib

        import "github.com/razshare/frizzante/frz"

        func Hello(c *frz.Connection) {
            c.SendView(frz.View{
                Name: "Hello",
                RenderMode: frz.RenderModeServer, // Renders on server.
            })
        }
        ```
        :::tip
        While using `RenderModeServer` the view won't serve a JavaScript bundle,
        but you can still use the `<svelte:head>` special
        tag in order to load scripts dynamically.
        ```svelte
        <svelte:head>
            <script type="text/javascript" src="https://some.cdn/file.js" />
        </svelte:head>
        ```
        :::
    </TabItem>
    <TabItem label='RenderModeClient'>
        The view is rendered only on the client by loading a JavaScript bundle asynchronously.
        ```go
        //lib/routes/handler.go
        package lib

        import "github.com/razshare/frizzante/frz"

        func Hello(c *frz.Connection) {
            c.SendView(frz.View{
                Name: "Hello",
                RenderMode: frz.RenderModeClient, // Renders on client.
            })
        }
        ```
    </TabItem>
</Tabs>


## Embedding views

You can embed views into your binary so that you don't have to carry around your `app/dist` directory.

Use `WithEfs()` to set the server's embedded file system.

```go
package main

import (
    "embed"
    "github.com/razshare/frizzante/frz"
)

//go:embed app/dist
var efs embed.FS

func main() {
    frz.NewServer().
        WithEfs(efs).
        WithPublicRoot("app/dist/client").
        WithViewServer("app/dist/server.js").
        WithViewIndex("app/dist/client/index.html").
        Start()
}
```


Now `SendFileOrElse()` will also take into consideration the embedded file 
when looking files up.

:::tip
The **host** file system **has precedence** over the embedded file system.

This means you can add a custom `app/dist` directory nearby your binary in order to serve
files that were not originally embedded into the binary.
:::

