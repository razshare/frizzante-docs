...
Generate frizzante's utilities files.

```sh
make generate
```

This will create a new **frz** directory.
...
Reference `dist/client` in your server definition using `WithPublicRoot()`,
`dist/client/index.html` using `dist/client/index.html()` and
`dist/server/server.js` using `WithViewServer()`.
...
:::note
These `dist/*` files will be automatically created whenever you build the project or start development mode.
:::
...
<Tabs>
    <TabItem label='RenderModeFull'>
        The view will be rendered on both the server and the client.<br/>
    ...
    <TabItem label='RenderModeServer'>
        The view will be rendered only on the server.<br/>
