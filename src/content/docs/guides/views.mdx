...
For 99% of use cases a limit of 1 runtime (the default) is more than enough, after all, the Svelte compiler
is simply concatenating strings together when rendering pages on the server.


Make sure you modify this field based on actual performance measurements.
:::
:::
...
:::tip
If the view **Name** is not present in neither `app/exports/server.ts` or `app/exports/client.ts`, `send.View()` 
invokes the [render function](#_top), which will ultimately resolve into a **Noop**.

The default starter template comes with a client and server routing configuration 
out of the box, which you can find in [app/frizzante/core/components/*](https://github.com/razshare/frizzante/tree/main/template/app/frizzante/core/components).

Although it's discouraged to do so, you can modify this configuration in order to customize what happens when the view is not found
and thus avoid **Noops**.
:::
