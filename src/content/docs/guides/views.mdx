...
func View(c *client.Client) {
    send.View(c, view.View{     // Sends view.
       ...
       Data: map[string]string{ // Sets view data, which will be injected into the svelte component.
...
:::caution
Some modes are only available while using a specific rendering function, for example `RenderModeFull` and `RenderModeServer` 
...
While it's true that you can simply choose to only use `RenderModeClient` when using an [ssr function](#ssr-function), your binary will still bundle the required JavaScript engine, even if you're not making use of it.
