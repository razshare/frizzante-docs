...
The ssr function uses a JavaScript engine in order to render Svelte components on the fly and then return the result.
...
var dev = os.Getenv("DEV") == "1"                    // If in dev mode we want to send files
                                                     // directly from disk in order to get
                                                     // immediate feedback from Vite.

func View(client *client.Client) {
    send.FileOrElse(client, dev, func () {           // Attempts to send requested file, or else...
       send.View(client, view.View{Name: "Welcome"}) // ...sends view "Welcome".
    })
}
```
...
:::note
Is the second parameter of `send.FileOrElse()` is set to `true`, 
then the file will be served directly from the host file system (**disk**), otherwise it will be served from the embedded file system.
:::
...
eliminated from your final binary, reducing its size from a minimum of 24MB, to a minimum of 10MB.