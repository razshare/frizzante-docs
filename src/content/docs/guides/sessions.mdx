...
:::note
Is the user provides a `session-id` cookie, `Session()` will try and find that session.

If the session doesn't exist, it will create a new session with a new id.
...
When invoking `Session()`, you're given back 2 values, a `state` and an `operator`.

Use the `operator` to save, reload and destroy the session.
