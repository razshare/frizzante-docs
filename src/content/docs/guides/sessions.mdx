...
func Welcome(con *connections.Connection) {
    session := sessions.Start[State](con)   // Starts the session with zero state.
}
```
...
func Welcome(con *connections.Connection) {
    session := sessions.Start[State](con)          // Starts the session.
    sessions.Load(session)                         // Loads the session's state (not necessary, read below).
    con.SendMessage("Hello " + session.State.Name) // Sends text.
}
```
...
func Welcome(con *connections.Connection) {
    session := sessions.Start[State](con) // Starts the session.
    sessions.Destroy(session)             // Destroys the session.
}
```
...
func main() {
    ...
    server.SessionArchive = &CustomArchive{} // Overwrite the default session archive.
                                             ...
                                             // in order to save and load session, for example,
                                             // using a database.
}
```
...
So given the interface from above, the following would be description of what the default implementation does, at a high level.
