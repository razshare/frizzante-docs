...
This will create and start the client's session.

:::note
The session is retrieved based on the client's `session-id` cookie.

If the client doesn't have a `session-id` cookie, `Start()` creates a 
new one instead and sends it to the client.
:::

:::danger
Since `Start()` may send a cookie, it is important to remember that
the order in which data is sent to the client matters.
...
func Welcome(connection *connections.Connection) {
    ...
    connection.SendMessage("Hello " + session.State.Name) // Sends message..
}
```
...
func main() {
    ...
    server.SessionArchive = &CustomArchive{} // Overwrites the default session archive.
                                             // The default session archive saves and loads sessions from disk. 
                                             // You can overwrite this archive in order to save and load sessions, 
...
And subsequently, where `domain` is the client's session id and `key` is always set to `"session.json"`.
