...
Use `req.ReceiveHeader()` to retrieve header fields and `res.SendHeader()` to send them.
...
Path fields can then be retrieved with `req.ReceivePath()`.
...
func GetHandler(req *frizzante.Request, res *frizzante.Response) {
    ...
    for *alive {                    // Loop until request gets cancelled.
        res.SendMessage("hello")    // Send text.
        time.Sleep(time.Second)     // Sleep for 1 second.
    }
}
```
