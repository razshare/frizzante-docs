...
type Hook func(client *client.Client, next func()) // Defines a hook function type which will be used by the middleware.
...
func Apply(mid *Middleware, routes []route.Route) {
    for _, r := range routes {                                   // For each route...
        ...
        r.Handler = func(client *client.Client) {                // Assigns a new wrapper route handler.
            var quit = true                                      // Creates flag used to interrupt the chain.
            for _, hook := range mid.Hooks {                     // Iterate over hooks.
...
var mid = &middleware.Middleware{                  // Creates middleware.
    Hooks: []middleware.Hook{                      // Creates hooks.
        func(client *client.Client, next func()) { // Adds hook.
                                                   // Logic goes here.
        },
    },
}
