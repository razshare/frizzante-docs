...
Although the application itself is simple, there are a lot of things that might need some explanation.
...
This is important because this pattern will capture 
any request that is not captured by the other patterns.

This means that the default handler not only handles the default application view,
but it also handles all file requests.
...
`SendFileOrElse` will try find the file on the host file system first,
if the file is not found, it will try to find it in the binary embedded file system,
otherwise it falls back to the `Welcome` handler.

## Welcome Handler

All this handler does is simple send the `"Welcome"` view to the user using `SendView()`.
...
The keys are important, because they dictate the `Name` of the view,
as it the actual property `Name` of the view.
...
The `"Welcome"` view is simply laying down a header message and link pointing 
to `"/todos"`.
...
When the view is allowed to execute JavaScript in the client's browser,
the hyperlink will locally swap the view in inside the client's browser
instead of navigating away to the given path.
...
Although rare, a use might choose to disable JavaScript altogether on your
website.
...
not compatible with the user's browser, in which case the hyperlink will 
continue to work by falling back to its standard behavior.
:::
...
Where checking an item sends a form to `/check` and unchecking an items sends it instead to `/uncheck`.
