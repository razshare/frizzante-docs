...
The `Default` handler is exposed with a `GET /` pattern, which 
captures any request that is not captured by the other patterns.

This means that the `Default` handler not only handles the main 
application view, but it also handles all file requests.
