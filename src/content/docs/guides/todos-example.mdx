...
not compatible with the user's browser. In that case, there's stilla good change that
...
thus the final `session.State` property will always contain the 5 todo items when initialized.
...
func Add(con *connections.Connection) {
	session := sessions.Start(con, lib.InitialState())
	defer operator.Save(session.State)
