...
```go
go install main/lib/core@latest
```
...
The frizzante repository comes with a starter template project, located at `template/project`, which is used for both local development and it is also embedded into the CLI for generating new projects interactively.

You most likely will want to try out your `frizzante` changes locally.

By default, your local template project will pull the `frizzante` package from the public remote repository.\
You can link the template project to your local version of `frizzante` instead, in order to have immediate feedback.

Navigate to your `template/project` directory and modify `template/project/go.mod` to replace the remote `frizzante` dependency with the local one using `replace` syntax.
...
// ...
replace main/lib/core => /home/user1/path/to/frizzante
require main/lib/core v1.17.2
// ...
```

This will make it so that changes to your local `frizzante` project are immediately picked up when building your local template project.
...
```go
require main/lib/core v1.17.3
```

And `v1.17.3` doesn't actually exist yet, your package request will be rejected.\
...
The whole test suite makes use of one single server, which is initialized in [init_test.go](https://main/lib/core/blob/main/init_test.go).
